...
/**
 * <p>
 * A pin in libbulldog is solely a management container. It contains features
 * that can be activated on it.</p>
 *
 * <p>
 * E.g. one might choose to use the Pin as a {@code DigitalOutput} or a
 * {@code DigitalInput}.</p>
 *
 * <p>
 * The pin contains the necessary information to be set up according to the
 * user's wishes. If the information of this class is not enough, then board
 * implementations derive from it. It ensures that only one feature can be
 * activated at a time. If asynchronous or blocking operations are currently in
 * progress on another feature, then the pin will deny any other feature
 * activation during that time.</p>
 *
 * <p>
 * Mostly, the pin is just used to grab a feature that is available on it:<br/>
...
public class Pin {
    ...
    /**
     * The name given to the pin Usually a human readable name that is easy to
     * remember.
     */
    private String name = null;

    /**
     * The user can give an alias to the pin, so he can identify the pin by his
     * own name and is not dependent on the name given to it by the system.
     */
    ...
    /**
     * If the pin is blocked an no other feature can be activated, then this
     * variable hold the feature that currently blocks ths pin.
     */
    ...
    /**
     * Adds the feature to the pin. This method is used to provide a pin
     * instance with all the features that it is capable of.
     *
    ...
    /**
     * Removes a feature from a pin. More precisely, it removes the first
     * occurrence of a feature class from the pin.
     *
    ...
    /**
     * This is a very important method. It is the standard way to configure a
     * pin for usage with a desired feature (e.g. PWM, DigitalIO).
     *
    ...
    /**
     * Blocks a pin. While a pin is blocked, not other pin feature can be
     * activated on the pin until it unblocks the pin.
     *
     ...
     *
     * @param blocker the feature that currently blocks the pin. If the pin is
     * blocked, but not by the pin feature given in the argument, then a
     * PinBlockedException will be thrown.
     */
    ...
    /**
     * Grabs a feature from the pin, if it is available. First, the method will
     * look for a pin feature class that exactly matches the class specified in
     * the argument. If that cannot be found, it returns a pin feature that is
     * at least assignable from the desired feature class.
     *
     * @param <T> the concrete type of the pin feature
     ...
     *
     * Usually this corresponds with the GPIO pin number of the package or
     * board.
     *
     * @return the address
     */
    ...
    /**
     * Gets the name of the pin. Usually a combination of port name and pin
     * index.
     *
    ...
    /**
     * Gets the alias of the pin. The alias is an alternate name that can be set
     * by the user. {@link Pin#setAlias(String) setAlias}.
     *
    ...
    /**
     * Sets the alias of the pin. This is an alternate name that can be set by
     * the user.
     *
     ...
     *
     * @param featureClass the feature class
     * @return true, if the feature class is available on the pin, false
     * otherwise.
     */
    ...
    /**
     * This method activates a feature on the pin. This includes tearing down
     * any active feature (if it is not blocking) and setting up the new
     * feature.
     *
    ...
    /**
     * This method deactivates a feature. The feature will be torn down and the
     * {@code activeFeature()} will be set to {@code null}.
     *
     * @param feature the feature
     */
    ...
    /**
     * This method fires the feature activating event. It denotes that a feature
     * activation has begun. the feature is not ready for use yet. All
     * registered {@link FeatureActivationListener FeatureActivationListeners}
     * will react on it.
     *
    ...
    /**
     * This method fires the feature activated event. It denotes that a feature
     * has been fully activated and is ready for use. All registered
     * {@link FeatureActivationListener FeatureActivationListeners} will react
     * on it.
     *
     * @param feature the feature that is currently being activated
     */
    ...
    /**
     * This method fires the feature deactivating event. It denotes that a
     * feature deactivation has begun. the feature is not torn down yet. All
     * registered {@link FeatureActivationListener FeatureActivationListeners}
     * will react on it.
     *
     * @param feature the feature that is currently being activated
    ...
    /**
     * This method fires the feature deactivated event. It denotes that a
     * feature deactivation has finished. the feature is completely torn down.
     * All registered
     * {@link FeatureActivationListener FeatureActivationListeners} will react
     * on it.
     *
    ...
    public int hashCode() {
        int hash = 7;
        hash = 11 * hash + address;
        hash = 13 * hash + name.hashCode();
        hash = 17 * hash + alias.hashCode();
        hash = 19 * hash + port.hashCode();
        hash = 23 * hash + indexOnPort;
        return hash;
    }
